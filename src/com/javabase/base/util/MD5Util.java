...
public class MD5Util {
	...
	public static String encrypt(String xmlStr) {
		...
		byte[] md5Hasn = null;
		try {
			md5Hasn = MD5Util.MD5Hash(encrypt, 0, encrypt.length);
		}
		catch (Exception e) {
			e.printStackTrace();
		}
		// 组合消息体
		byte[] totalByte = MD5Util.addMD5(md5Hasn, encrypt);

		// 取密钥和偏转向量
		byte[] key = new byte[8];
		byte[] iv = new byte[8];
		getKeyIV(MD5Util.key, key, iv);
		...
		byte[] temp = null;
		try {
			temp = MD5Util.DES_CBC_Encrypt(totalByte, deskey, ivParam);
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static String decrypt(String xmlStr) {
		try {
			...
			byte[] temp = null;
			try {
				temp = MD5Util.DES_CBC_Decrypt(encBuf, deskey, ivParam);
			}
			catch (Exception e) {
				e.printStackTrace();
			}
