...
public class BeanUtil {
	 ...
	 */
	public static void copyFields(Object src, Object des, boolean copyNull,
			String[] srcFieldNames, String[] desFieldNames) {
		for (int i = 0; i < srcFieldNames.length; i++) {
			...
			try {
				Method method = src.getClass().getMethod(methodName, null);
				Class returnType = method.getReturnType();
				Object returnValue = method.invoke(src, new Object[] {});
				if (!copyNull && returnValue == null) {
					continue;
				}
				String setInvokeMethodName = "set"
						+ (desFieldName.substring(0, 1).toUpperCase() + desFieldName
								.substring(1, desFieldName.length()));
				String getInvokeMethodName = "get"
						+ (desFieldName.substring(0, 1).toUpperCase() + desFieldName
								.substring(1, desFieldName.length()));
				Method getInvokeMethod = des.getClass().getMethod(
						getInvokeMethodName, null);
				Class desReturnType = getInvokeMethod.getReturnType();

				Method invokeMethod = des.getClass().getMethod(
						setInvokeMethodName, new Class[] { desReturnType });
				invokeMethod.invoke(des,
						new Object[] { returnValue.getClass().getName().equals(
								desReturnType.getName()) ? returnValue
								: desReturnType.cast(returnValue) });
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	 ...
	 */
	public static void copyFields(Object src, Object des, boolean copyNull,
			boolean copySetType, boolean copyDateType) {
		final String get = "get";
		final String set = "set";
		final String is = "is";
		...
		for (int i = 0; i < methods.length; i++) {
			...
			if ((methodName.startsWith(get) && !methodName.equals("getClass"))
				...
				try {
					...
					if (!copySetType
							&& !copyDateType
							&& !"Set".equals(method.getReturnType()
									.getSimpleName())
							&& !"Date".equals(method.getReturnType()
									.getSimpleName())) {

						Method invokeMethod = desClass.getMethod(
								invokeMethodName, new Class[] { method
										.getReturnType() });
						Object result = method.invoke(src, new Object[] {});
					...
					for (Method method2 : methoddes) {
						...
						if (method2.getName().equals(invkemethod)) {
							...
							if (method.getReturnType().getName().equals(
								...
								Object object = method.invoke(src, null);
								if (object != null) {
	 ...
	 *            是否拷贝为空对象
	 */
	public static List copyFields(List src, Object des, boolean copyNull) {
