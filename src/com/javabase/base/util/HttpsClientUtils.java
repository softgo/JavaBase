...
/**
 * 
 * 去掉https请求中的证书认证,可以不用证书认证就可以直接去访问
 * 
 * @author bruce
 *
 */
public class HttpsClientUtils {
    ...
    static {
        ...
        conMgr.setMaxTotal(200);                                //设置连接池线程最大数量
        conMgr.setDefaultMaxPerRoute(conMgr.getMaxTotal());     //设置单个路由最大的连接线程数量
    ...
    public static String getUrlContent(HttpUriRequest httpUriRequest) {
        ...
        try {
            LOGGER.info(httpUriRequest);

            //Header[] headers2 = httpUriRequest.getAllHeaders();
            ...
            //System.out.println("----------------------------------------Uncompressed size:" + responseContent.length());
