 ...
 * 
 */
public abstract class MonGoDAO<T, K> implements DAO<T, K> {
	//框架基础类.
	...
	public MonGoDAO() {
		...
		System.out.println("poolName="+poolName+",dbName="+dbName);
		Datastore ds = null;
		if(dbName.isEmpty()){
			ds = MonGoHelper.getDatastore(poolName);
		} else{
			ds = MonGoHelper.getDatastore(poolName, dbName);
		}
		if(ds == null){
	 ...
	 * @return
	 */
	public Query<T> getQuery(Map<String, ?> params) {
		Query<T> query = createQuery();
		if(params != null){
			for(Entry<String, ?> entity : params.entrySet()) {
				String name = entity.getKey();
				Object value = entity.getValue();
				String[] col_ope = name.split("_@");
				if(col_ope.length >= 2){
					if(col_ope[1].equals("like")){
						query.filter(col_ope[0], java.util.regex.Pattern.compile(value.toString()));
					}else if(col_ope[1].equals("begin")){
						query.field(col_ope[0]).greaterThanOrEq(value);
					}else if(col_ope[1].equals("end")){
						query.field(col_ope[0]).lessThanOrEq(value);
					}else if(col_ope[1].equals("not")){
						query.field(col_ope[0]).notEqual(value);
					}else if(col_ope[1].equals("contain")){
						query.field(col_ope[0]).hasThisOne(value);
					}else if(col_ope[1].equals("in")){
						query.field(col_ope[0]).hasAnyOf((Iterable<?>)value);
					}else if(col_ope[1].equals("noneof")){
						query.field(col_ope[0]).hasNoneOf((Iterable<?>)value);
					}else if(col_ope[1].equals("or")){ //或者查询
						@SuppressWarnings("unchecked")
						Map<String,?> temp = (Map<String,?>)value;
						if(temp!=null && temp.size()>0){
							List<CriteriaContainer> collect = new ArrayList<CriteriaContainer>(); 
							for(Entry<String,?> entity_temp : temp.entrySet()){
								collect.add(query.criteria(entity_temp.getKey()).equal(entity_temp.getValue()));
							}
					...
					if(name.equals("order")){
						...
						query.field(col_ope[0]).equal(value);
					}
				}
			}
		}
	 ...
	 * @return
	 */
	public int update(K id, Map<String,?> params) {
