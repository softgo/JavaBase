...
public class ExcelBase 
{
	...
	public ExcelBase(String sheetName, HttpServletResponse response)
	{
    	...
    	dataStyle_TURQUOISE.setFillForegroundColor(HSSFColor.LIGHT_TURQUOISE.index);
	 ...
	 * @param region		记录合并单元格的坐标 (起始坐标为 0,0)
	 * @param field_one		Excel列表头第一行的内容
	 * @param field_two		Excel列表头第二行的内容
	 * 
	 * 例:
	 * 	int[][] region = 
		{ 
			{ 0, 0, 1, 0}, 	表示 0行0列 与 1行0列 两个单元格合并
			{ 0, 1, 1, 1},	表示 0行1列 与 1行1列 两个单元格合并
			{ 0, 2, 1, 2},	...
			{ 0, 3, 1, 3},	...
			{ 0, 4, 0, 7},	表示 0行4列 0行5列 0行6列 0行7列 四个单元格合并
			{ 0, 8, 0, 11},	...
		}; 
		String[] field_one = {"所属大区", "公司等级", "公司名称", "人数", "按年龄分布划分", "按年龄分布划分", "按年龄分布划分", "按年龄分布划分", "按年龄分布划分比例", "按年龄分布划分比例", "按年龄分布划分比例", "按年龄分布划分比例"};
		String[] field_two = {"所属大区", "公司等级", "公司名称", "人数", "22岁以下", "22岁~25岁", "25岁~28岁", "28岁以上", "22岁以下比例", "22岁~25岁比例", "25岁~28岁比例", "22岁~25岁比例"};
	 
	   执行效果如下：
	   
		所属大区 	公司级别 	公司名称 	人数 			按年龄分布划分 							按年龄分布划分比例 
									  22岁以下 22岁~25岁 25岁~28岁 28岁以上   22岁以下比例 22岁~25岁比例 25岁~28岁比例 22岁~25岁比例 
	 */
	public void createHead(int[][] region, String[] field_one, String[] field_two)
	{
		//创建报表头(2行)
		this.createRow();
		...
		{
			Region the_region = new Region(region[i][0], (short)region[i][1], region[i][2], (short)region[i][3]);
    	...
    	try
    	{
	    	...
	    	for(int i = 0; i < list.size(); i++)
	    	{
	    		...
	    		for(int j = 0; j < obj.length; j++)
	    		{
	    			...
	    			if(obj[j] == null)
	    				cell.setCellValue("");
	    			else if(j==0 && (obj[j].toString().equals("1") || obj[j].toString().equals("0"))) 
	    				cell.setCellValue("--");
		...
		try
    	{
	    	...
	    	for(int i = 0; i < Array.length; i++)
	    	{
	    		...
	    		for(int j = 0; j < Array[i].length; j++)
	    		{
	    			...
	    			//cell_value = obj[j].toString().substring(obj[j].toString().indexOf(">")+1, obj[j].toString().lastIndexOf("<"));
	    			//cell_style = obj[j].toString().substring(obj[j].toString().indexOf("#")+1,obj[j].toString().indexOf("#")+7);
