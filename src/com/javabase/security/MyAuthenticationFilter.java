...
public class MyAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
	...
	@Autowired
	private LoginLogsService userLoginLogService;
...
//		System.err.println(" ---------------  MyAuthenticationFilter attemptAuthentication--------------- ");
		...
		// 验证用户账号与密码是否正确
		SysUsers users = this.userDao.findByName(username);
		if (users == null || ! users.getSysUserPass().equals(password)) {
			BadCredentialsException exception = new BadCredentialsException(
		...
		loginLogs.setSysUserId(users.getSysUserId());
		loginLogs.setSysUserName(users.getSysUserName());
		loginLogs.setLoginIp(Common.toIpAddr(request));
		System.out.println(" userId----" + users.getSysUserId() + "---ip--" + Common.toIpAddr(request));
		userLoginLogService.addOne(loginLogs);
