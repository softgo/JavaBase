...
import com.javabase.base.mail.util.EmailUtils;
...
public class SpringTaskController {
	...
	@Autowired
	private SysServerInfosService serverInfoService;
	...
	public void action() throws Exception {
		Properties prop = PropertiesUtils.getProperties(PropertiesUtils.propsFilePath);
		if (null != prop) {
			String data = null;
			try {
	...
	public void task() throws Exception {
		...
		if (!Common.isEmpty(cpubool) || !Common.isEmpty(jvmbool) || !Common.isEmpty(rambool)) {
			try {
				...
				// 保存预警信息
				SysServerInfos vo = new SysServerInfos();
				...
				vo.setOperatorTime((Timestamp)new Date());
				vo.setMark(mark);
				serverInfoService.addOne(vo);
