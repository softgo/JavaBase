...
<body>
<form id="fenye" name="fenye" action="<%=request.getContextPath()%>/background/souyueUsers/find.html" method="post">
                    ...
                    <td>
	                    <div class="box_bottom pb5 pt5 pr10" style="border-top:0px solid #dadada;">
			              <div class="search_bar_btn" style="text-align:right;">
	                         ...
	                         <sec:authorize ifAnyGranted="ROLE_souyue_users_addUI">
	                            <input type="button" name="button" class="btn btn82 btn_add" value="新增" onclick="GOTO('<%=request.getContextPath()%>/background/souyueUsers/addUI.html')">
	                         </sec:authorize>
	                         <sec:authorize ifAnyGranted="ROLE_souyue_users_delete">
            ...
            <div class="box span10 oh">
              <table class="list_table" width="100%" cellspacing="1" onmouseover="changeto()"  onmouseout="changeback()">
			          ...
			          <c:forEach var="key" items="${pageView.records}">
						...
						<td align="center" ><a href="<%=request.getContextPath()%>/background/souyueUsers/findById.html?objId=${key.userId}&type=0">${key.userId}</a></td>
						<td align="center" >${key.userName}</td>
						<td align="center" >${key.userPass}</td>
						<td align="center" >${key.nickName}</td>
						<td align="center" >${key.userPhone}</td>
						<td align="center" >${key.userMail}</td>
						<td align="center" >${key.userQq}</td>
						<td align="center" >${key.userSex}</td>
						<td align="center" >${fn:substring(key.registerTime,0,19)}</td>
						<td align="center" >${key.status}</td>
						<td align="center" >
						
						<sec:authorize ifAnyGranted="ROLE_souyue_users_info">
							<img src="<%=request.getContextPath()%>/images/admin.gif" width="10" height="10" />
							<a href="<%=request.getContextPath()%>/background/souyueUsers/findById.html?objId=${key.userId}&type=0">
							详细信息</a>
						</sec:authorize>
						<sec:authorize ifAnyGranted="ROLE_souyue_users_edit">
							<img src="<%=request.getContextPath()%>/images/edt.gif" width="12" height="12" />
							<a href="<%=request.getContextPath()%>/background/souyueUsers/findById.html?objId=${key.userId}&type=1">编辑</a>
						</sec:authorize>
						<sec:authorize ifAnyGranted="ROLE_souyue_users_delete">
							<img src="<%=request.getContextPath()%>/images/del.gif" width="16" height="16" />
							<a href="javascript:void(0);" onclick="deleteId('<%=request.getContextPath()%>/background/souyueUsers/deleteById.html','${key.userId}')">删除</a>
						</sec:authorize>
